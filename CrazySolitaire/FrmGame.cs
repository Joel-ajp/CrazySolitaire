...
namespace CrazySolitaire {
    public partial class FrmGame : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // Subscribe and set initial label
        ...
        public static void CompleteRunDrop(TableauStack target)
        {
            ...
            foreach (var card in CurDragRun)
            {
                ...
                if (!counted)
                {
                    Game.RegisterMove(card, CardDraggedFrom, target);
                    counted = true;
                }
            }
        ...
        {
            if (lblCoinCount.InvokeRequired)
            {
                lblCoinCount.BeginInvoke(new Action(() => lblCoinCount.Text = $"Coins: {coins}"));
            }
            else
            {
